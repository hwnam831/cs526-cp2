...
void runTest(int argc, char** argv) {
...

	{
		...
		// execute the kernel
		demosaic_naive<<< grid, threads >>>(d_A, d_C, WIDTH_A);
...

	{
		...
		CUT_SAFE_CALL(cutStartTimer(timer));
		// execute the kernel
		demosaic_coalesced<<< grid, threads >>>(d_A, d_C, WIDTH_A);
		// stop and destroy timer
		cudaThreadSynchronize();
		CUT_SAFE_CALL(cutStopTimer(timer));
...

	{
		...
		CUT_SAFE_CALL(cutStartTimer(timer));
		// execute the kernel
		demosaic_opt<<< grid, threads >>>(d_A, d_C, WIDTH_A);
		// stop and destroy timer
		cudaThreadSynchronize();
		CUT_SAFE_CALL(cutStopTimer(timer));
